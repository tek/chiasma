...
import Polysemy.Membership (transformSem)
import Polysemy.Internal.RowTransformer (underRowMany, raiseRow)
...
insertAfter ::
  ...
  Sem (left ++ e : r) a
insertAfter =
  transformSem (underRowMany (singList @left) (raiseRow @r @e))
